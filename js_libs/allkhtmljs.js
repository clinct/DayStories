//EDITED LINE #897: KHTML: Please no zoom on hold!

//
//  GPX Rendering
//

function kGPX(dom,style){
    this.dom=dom;
    this.style=style;
    this.canvasse=new Array();
    var sw=new kPoint(90,180);
    var ne=new kPoint(-90,-180);
    this.bounds=new kBounds(sw,ne);
    ...
    this.init=function(mapObj){
        ...
        if(this.style){
            ...
            style.addStyle("strokeStyle","blue");
        ...
        for(var i=0;i<trksegs.length;i++){
            ...
            for(var j=0; j < trkpts.length;j++){
                ...
                var lng=trkpt.getAttribute("lon");
                //console.log(lat,lng);
                var point=new kPoint(lat,lng);
                points.push(point);
                this.makeBounds(point);
            }
            this.canvasse[i]=new kPolyline(points,style);
                        this.canvasse[i].init(mapObj);      

        }
        var wpts=this.dom.getElementsByTagName("wpt");
        for(var h=0;h<wpts.length;h++){
            var wpt=wpts.item(h);
            var lat=wpt.getAttribute("lat");
            var lng=wpt.getAttribute("lon");
            var point=new kPoint(lat,lng);
            this.makeBounds(point);
        ...
        if(i==0){
            this.canvasse[i]=new kPolyline(waypoints,style);
                        this.canvasse[i].init(mapObj);      
            i++;
        }
        if(i > 1){
        for(var h=0;h<waypoints.length;h++){
                        var style=new kStyle();
                        style.addStyle("strokeStyle","black");
                        style.addStyle("fillStyle","yellow");
                        style.addStyle("lineWidth",1);
            var point=waypoints[h];
                        this.canvasse[i +h]=new kCircle(point,"4px",style);
                        this.canvasse[i +h].init(mapObj);

        }
        }
        /*
        if(i==1){
        if(this.start && waypoints[0]){
        var m=new kMarker(waypoints[0],this.start);
        mapObj.addOverlay(m);
        }
        if(this.stop && waypoints[waypoints.length-1]){
        var m=new kMarker(waypoints[waypoints.length-1],this.stop);
        mapObj.addOverlay(m);
        }
        }
        */
        ...
        */
                for(var canvas in this.canvasse){
                        var ctx=this.canvasse[canvas].render();
                }

        }
    this.makeBounds=function(point){
        if(this.bounds.getSW().getLng() > point.getLng()){
            this.bounds.getSW().setLng(point.getLng());     
        }
        if(this.bounds.getSW().getLat() > point.getLat()){
            this.bounds.getSW().setLat(point.getLat());     
        }
        if(this.bounds.getNE().getLng() < point.getLng()){
            this.bounds.getNE().setLng(point.getLng());     
        }
        if(this.bounds.getNE().getLat() < point.getLat()){
            this.bounds.getNE().setLat(point.getLat());     
        }
    ...
    this.getBounds=function(){
        ...
        this.clear=function(){
              ...
              try{
                        vectorEl=this.mapObj.mapParent.getElementsByTagName("svg").item(0);
                        while(vectorEl.firstChild){
                                vectorEl.removeChild(vectorEl.firstChild);
                        }
              }catch(e){ };
                try{
                        vectorEl=this.mapObj.mapParent.getElementsByTagName("vml").item(0);
                        while(vectorEl.firstChild){
                                vectorEl.removeChild(vectorEl.firstChild);
                        }
                }catch(e){ };
    ...
    this.description=function(){
        ...
        returnDiv.appendChild(this.metadata());
        returnDiv.appendChild(this.extensions());
        var rtes=this.dom.getElementsByTagName("rte");
        if(rtes.length==0){
            var div=document.createElement("div");
            var tn=document.createTextNode("no description");
            div.appendChild(tn);
            return div;
        }
        var table=document.createElement("table");
        var tbody=document.createElement("tbody");
        table.setAttribute("class","routingdescription");
        var rtepts=rtes.item(0).getElementsByTagName("rtept");
        for(var i=0; i < rtepts.length;i++){
            var d=makeItem(i,rtepts.item(i));
            tbody.appendChild(d);
        }
        ...
        this.descriptionList=function(){
                ...
                ul.setAttribute("class","routingdescription");
                var rtepts=rtes.item(0).getElementsByTagName("rtept");
                for(var i=0; i < rtepts.length;i++){
                        var li=makeItemLi(i,rtepts.item(i));
                        ul.appendChild(li);
                }
    ...
    function makeItem(num,el){
        var desc=el.getElementsByTagName("desc").item(0).firstChild.nodeValue;
        var lat=el.getAttribute("lat");
        var lng=el.getAttribute("lon");
        var tr=document.createElement("tr");
        var td1=document.createElement("td");
        var td2=document.createElement("td");
        var tn1=document.createTextNode(num+". ");
        var tn2=document.createTextNode(desc);
        td1.appendChild(tn1);
        td1.setAttribute("lat",lat);
        td1.setAttribute("lng",lng);
        td1.setAttribute("direct","yes");
        td2.setAttribute("lat",lat);
        td2.setAttribute("lng",lng);
        td2.appendChild(tn2);
        tr.appendChild(td1);
        tr.appendChild(td2);
        return tr;
    }
        function makeItemLi(num,el){
                var desc=el.getElementsByTagName("desc").item(0).firstChild.nodeValue;
        var li=document.createElement("li");
        var tn=document.createTextNode(desc);
        li.appendChild(tn);
        return li;
    ...
    this.metadata=function(){
        var returnDiv=document.createElement("div");
        try{
            var author=this.dom.getElementsByTagName("author").item(0);
            var name=author.getElementsByTagName("name").item(0).firstChild.nodeValue;
            var link=author.getElementsByTagName("link").item(0).getAttribute("href");
            var d=document.createElement("div")
            var a=document.createElement("a")
            a.setAttribute("href",link);
            var linktn=document.createTextNode(name);
            a.appendChild(linktn);
            var c=document.createTextNode("(C) ");
            var e=document.createTextNode(" routing");
            d.appendChild(c);
            d.appendChild(a);
            d.appendChild(e);
            returnDiv.appendChild(d);
        }catch(e){}
        return returnDiv;
    }
    this.extensions=function(){
        var returnDiv=document.createElement("div");
        returnDiv.setAttribute("class","gpxdesc");
        var extensions=this.dom.getElementsByTagName("extensions");
        for(var i=0; i < 1;i++){
            var extension=extensions.item(i);
            try{
                var extension=extensions.item(i);
                var distance=extension.getElementsByTagName("distance").item(0).firstChild.nodeValue;   
                var div=document.createElement("div");
                var rounddistance=Math.round(distance/100)/10;
                var tn=document.createTextNode("Distance: "+rounddistance+" km");
                div.appendChild(tn);
                returnDiv.appendChild(div);
            }catch(e){} 
                        try{
                                var extension=extensions.item(i);
                                var time=extension.getElementsByTagName("time").item(0).firstChild.nodeValue;
                                var div=document.createElement("div");
                otime=timeFormatter(time);
                                var tn=document.createTextNode("Time: "+otime);
                                div.appendChild(tn);
                                returnDiv.appendChild(div);
                        }catch(e){}
                        try{
                                var extension=extensions.item(i);
                                var start=extension.getElementsByTagName("start").item(0).firstChild.nodeValue;
                                var div=document.createElement("div");
                                var tn=document.createTextNode("Start: "+start);
                                div.appendChild(tn);
                                returnDiv.appendChild(div);
                        }catch(e){}
                        try{
                                var extension=extensions.item(i);
                                var end=extension.getElementsByTagName("end").item(0).firstChild.nodeValue;
                                var div=document.createElement("div");
                                var tn=document.createTextNode("End: "+end);
                                div.appendChild(tn);
                                returnDiv.appendChild(div);
                        }catch(e){}
                        try{
                var av=parseFloat(distance) / time /1000*3600;
                var fav=parseInt(av);
                                var div=document.createElement("div");
                var tn=document.createTextNode("AV-Speed: "+fav+" km/h");
                                div.appendChild(tn);
                                returnDiv.appendChild(div);
            }catch(e){}
        }
    ...
    function timeFormatter(time){
        ...
        if(d >0){
            timeString=timeString+d+" days ";
        }
        var h=Math.floor(time/60/60) - d*24;
        if(h >0){
            timeString=timeString+h+" hours ";
        }
        var m=Math.round(time/60) -h*60 -d*24*60;
        timeString=timeString+m+" minutes";
        return timeString;
    }
}
...
function kMarker(point, el) {
    ...
    if(isNaN(this.point.getLat()))return;
    if(isNaN(this.point.getLng()))return;
...
function kPoint(lat, lng) {
    ...
    this.memberOf=function(m){
    this.memberOfArray.push(m);
    }
    this.getMemberOfArray=function(){
    return this.memberOfArray;
    }
}
...
function kKML(dom){
            ...
            if(child.nodeType==1){
                switch(child.tagName){
                    case "Folder":
                        ...
                        var name=child.getElementsByTagName("name").item(0).firstChild.nodeValue;
                        var tn=document.createTextNode(name);
                        var hdiv=document.createElement("div"); 
                        hdiv.setAttribute("class","heading1");
                        hdiv.appendChild(radio);
                        hdiv.appendChild(tn);
                        //h4.appendChild(tn);
                        div.appendChild(hdiv);
                        div.appendChild(div2);
                        tree.appendChild(div);  
                        div2.appendChild(this.makeTree(child));
                        //console.log(name);
                        break;
                    case "Placemark":
                        var names=child.getElementsByTagName("name");
                        //this.addToMap(child);
                        if(names.length >0){
                            var name=names.item(0).firstChild.nodeValue;
                            var tn=document.createTextNode(name);
                            var radio=document.createElement("input");
                            radio.setAttribute("type","radio");
                            radio.setAttribute("checked","checked");
                            var hdiv=document.createElement("div");
                            hdiv.setAttribute("class","heading2");
                            hdiv.appendChild(radio);
                            hdiv.appendChild(tn);
                            tree.appendChild(hdiv);
                        }
                        break;
                    case "Style":
                        this.makeStyle(child);
                    default:
                }
            }
        }
    ...
    function makeRGBA(color){
        var a=parseInt(color.substr(0,2),16)/256;
        var b=parseInt(color.substr(2,2),16);
        var g=parseInt(color.substr(4,2),16);
        var r=parseInt(color.substr(6,2),16);
        return "rgba("+r+","+g+","+b+","+a+")";
    }

    this.makeStyle=function(child){
        var id=child.getAttribute("id");
        var style= new kStyle();
        try{
            var color=child.getElementsByTagName("color").item(0).firstChild.nodeValue;
            color=makeRGBA(color);
            style.addStyle("fillStyle",color);
        }catch(e){}
        style.addStyle("strokeStyle","black");
        style.addStyle("lineWidth",1);
        
        this.styleArray[id]= style;
        //alert("gut: "+id+" l: "+this.styleArray.length);
    }

    this.addToMap=function(dom){
    ...
    this.makePolygon=function(dom){
        var points=new Array();
        try{
            var coordinates=dom.getElementsByTagName("coordinates").item(0).firstChild.nodeValue;
            var id=dom.getElementsByTagName("styleUrl").item(0).firstChild.nodeValue;
            id=id.substr(1,id.length);
            var style=this.styleArray[id];
        }catch(e){
            return;
        }
        ...
        for(var i=0; i < coords.length;i++){
            ...
            //console.log("coord: "+coord);
                var ar=coord.split(",");
                var lat=parseFloat(ar[1]);
                var lng=parseFloat(ar[0]);
                var h=ar[2];
                //console.log(lat,lng);
                var point=new kPoint(lat,lng);
                points.push(point);
        ...
        this.canvasse[this.ref]=new kPolylineCanvas(points,style);
        //this.canvasse[this.ref].init();
        this.ref++;
    }

    this.init=function(mapObj){
                for(var canvas in this.canvasse){
                        var ctx=this.canvasse[canvas].init(mapObj);
                }
    }
    ...
    this.render=function(mapObj){
        var canvas=document.getElementsByTagName("canvas").item(0);
                ...
                ctx.clearRect(0,0,1000,1000);
                for(var canvas in this.canvasse){
                        var ctx=this.canvasse[canvas].render(mapObj);
            //alert(canvas);
                }
    }
    this.clear=function(){
    ...
    this.doc=dom.getElementsByTagName("Document").item(0);
    
    this.tree=this.makeTree(this.doc);
    this.addToMap(this.doc);
    document.getElementById("kmltree").appendChild(this.tree);
...
function kMeasure(){
    ...
    this.path;
    this.points=new Array();
    this.distanceTextDiv=document.createElement("div");
    this.distanceTextDiv.style.position="absolute";
    this.distanceTextDiv.style.fontSize="25px";
    var tn=document.createTextNode("distance: 0km");
    this.distanceTextDiv.appendChild(tn);
    this.distanceTextDiv.style.backgroundColor="white";
    this.distanceTextDiv.style.opacity=0.8;
    ...
    this.init=function(mapObj){
        this.mapObj=mapObj;
        mapObj.map.parentNode.appendChild(this.distanceTextDiv);
        var style=new kStyle();
        style.addStyle("stroke","red");
        this.path=new kPolyline(this.points,style);
        this.path.init(this.mapObj);
        this.path.vectorEl.parentNode.appendChild(this.path.vectorEl);


        Event.attach(mapObj.map.parentNode, "mousedown", this.down, this, false);
        Event.attach(window, "mousemove", this.move, this, false);
        Event.attach(this.path.vectorEl, "mousemove", this.move, this, false);
        Event.attach(window, "mouseup", this.up, this, false);
    }
    this.render=function(){
        if(!this.active)return;
        var points=new Array();
        for(var i=0; i < this.circlesArray.length;i++){
            this.circlesArray[i].render();
            var c=this.circlesArray[i];
            var p=c.point;
            points.push(p);
        }
        this.points=points;
    ...
    this.clear=function(){
        for(var i=0; i < this.circlesArray.length;i++){
    //      this.circlesArray[i].clear();
        }
        this.path.clear();
        while(this.circlesArray.lenght >0){this.criclesArray.pop()}
    }
    this.destroy=function(){
        this.clear();
        if(this.distanceTextDiv.parentNode){    
            this.distanceTextDiv.parentNode.removeChild(this.distanceTextDiv);
        }
        this.active=false;  
    }
    this.down=function(evt){
        ...
        if(target.tagName=="circle" ||target.tagName=="vml"){
            this.movinger=target.obj;
        }else{
            this.newpoint=this.mapObj.mouseToLatLng(evt);
            this.createPoint(this.newpoint);
            this.render();
        }
    ...
    this.move=function(evt){
        ...
        if(this.movinger){
            ...
            this.movinger.moveTo(p);
            this.movinger.render();
            this.render();
            
        }
        if(this.newpoint){
            var p=this.mapObj.mouseToLatLng(evt);
            var c=this.createPoint(p);
            this.newpoint=null;
            this.movinger=c;
            this.render();
        }
    ...
    this.createPoint=function(point){
        var style=new kStyle();
                ...
                style.addStyle("opacity","0.6");
                var c=new kCircle(point,"5px",style);
                c.init(this.mapObj);
                this.circlesArray.push(c);
        return c;
    }
    ...
    this.calculateDistance=function(){
        ...
        var l=(m+"").length;
        if(l > 3){  
            var t=Math.round(m/10)/100+" km";
        }
        if(l > 4){  
            var t=Math.round(m/100)/10+" km";
        }
        if(l > 5){  
            var t=Math.round(m/1000)+" km";
        }
        this.distanceTextDiv.firstChild.nodeValue="distance: "+t;
    }
}
...
function kOSM(dom,style){
    this.dom=dom;
    this.nodes=new Array();
    this.ways=new Array();
    this.style=style;

    function makeStyleAge(timestamp,version){
        var style=new kStyle();
        style.addStyle("strokeStyle","rgba(0,0,0,0)");
        style.addStyle("fillStyle","rgba(0,0,0,0)");
        style.addStyle("lineWidth","2");
        var time=new Date();
        time.setISO8601(timestamp);
        var now=new Date();
        var t=Math.floor((now.getTime() - time.getTime())/24/3600/1000);
        t=t;
        var color="yellow";
        if(version==1){
            var color="red";
        }
        style.addStyle("strokeStyle",color);
        return style;
    }

    function makeStyle(tags){
        var style=new kStyle();
        style.addStyle("strokeStyle","rgba(0,0,0,0)");
        style.addStyle("fillStyle","rgba(0,0,0,0)");
        style.addStyle("lineWidth","1");
//      style.addStyle("strokeStyle","red");
        ...
        //  console.log("----"+tag["k"]+":"+tag["v"]);
            ...
            switch(tag["k"]){
                case "highway":
                    ...
                    switch(tag["v"]){
                        case "primary":
                            style.addStyle("strokeStyle","pink");
                            style.addStyle("lineWidth","6");
                            break;
                        case "secondary":
                            style.addStyle("lineWidth","4");
                            style.addStyle("strokeStyle","orange");
                            break;
                        case "tertiary":
                            style.addStyle("lineWidth","4");
                            style.addStyle("strokeStyle","yellow");
                            break;
                        case "residential":
                            style.addStyle("lineWidth","4");
                            style.addStyle("strokeStyle","#e0e0e0");
                            return style;
                            break;
                        case "track":
                        case "path":
                        ...
                        case "pedestrian":
                            style.addStyle("lineWidth","4");
                            style.addStyle("strokeStyle","grey");
                            return style;
                            break;
                        case "cycleway":
                        default:
                ...
                case "building":
                    ...
                    switch(tag["v"]){
                        case "yes":
                            style.addStyle("strokeStyle","grey");
                            style.addStyle("lineWidth","1");
                            style.addStyle("fillStyle","lightgrey");
                            return style;
                            break;
                        default:
                    //console.log("----"+tag["k"]+":"+tag["v"]);
                            style.addStyle("fillStyle","rgba(127,0,0,0.0)");
                    }
                case "amenity":
                    break;
                case "leisure":
                    switch(tag["v"]){
                        case "park":
                            //console.log("----park");
                            style.addStyle("strokeStyle","green");
                            style.addStyle("lineWidth","1");
                            style.addStyle("fillStyle","rgba(0,127,0,0.5)");
                            style.addStyle("fillStyle","green");
                            return style;
                            break;
                        default:
                            //style.addStyle("fillStyle","rgba(127,0,0,0.0)");
                ...
                case "landuse":
                    style.addStyle("fillStyle","rgba(127,127,127,0.3)");
                default:
                    //console.log("++++"+tag["k"]);
                            //style.addStyle("fillStyle","rgba(127,0,0,0.0)");
    ...
    this.addNode=function(c,color){
        if(c.tagName!="node"){
            return;
        }
        var lat=c.getAttribute("lat");
        var lng=c.getAttribute("lon");
        var id=c.getAttribute("id");
        if(!this.nodes[id]){
            this.nodes[id]=new Array();
            this.nodes[id]["point"]=new kPoint(lat,lng);
            this.nodes[id]["tags"]=new Array();
            var tags=c.getElementsByTagName("tag");
            for(var i=0;i< tags.length;i++){
                var tag=tags.item(i);
                var v=tag.getAttribute("v");
                var k=tag.getAttribute("k");
                this.nodes[id]["tags"].push(new Array(k,v));
            }
            if(tags.length > 0){
                color="green";
            }
            //console.log(this.nodes[id]["point"].getLat(),lng,lat);
            if(color){
                this.nodes[id]["color"]=color;
            }
        }
        
        //console.log(id,lat,lng,color);
    }
    this.addWay=function(c){
        var nodes=c.getElementsByTagName("nd");
        if(nodes.length==0){
            var serializer = new XMLSerializer();
            var xml = serializer.serializeToString(c);
        ...
        var id=c.getAttribute("id");
        this.ways[id]=new Array();
        this.ways[id]["points"]=new Array();
        for(var i=0;i< nodes.length;i++){
            var nd=nodes.item(i);
            //console.log(nd.tagName);
            var ref=nd.getAttribute("ref");
            this.ways[id]["points"][i]=this.nodes[ref]["point"];
        }
        
        this.ways[id]["tags"]=new Array();
        var tags=c.getElementsByTagName("tag");
        for(var i=0;i< tags.length;i++){
            var tag=tags[i];
            var tagArray=new Array();
            tagArray["k"]=tag.getAttribute("k");
            tagArray["v"]=tag.getAttribute("v");
            this.ways[id]["tags"][i]=tagArray;
        }
        this.ways[id]["timestamp"]=c.getAttribute("timestamp");
        this.ways[id]["version"]=c.getAttribute("version");
    }
    
    this.canvasse=new Array();
    this.init=function(mapObj){
        this.mapObj=mapObj;
            //console.log("so cool: " +this.ways.length);
        for(var ref in this.ways){
            if(!this.style){
                //var style=makeStyle(this.ways[ref]["tags"]);
                var style=makeStyleAge(this.ways[ref]["timestamp"],this.ways[ref]["version"]);
            }else{
                var style=this.style;
            }
            var way=this.ways[ref];
            var points=way["points"];   
            //console.log(style.getStyle("fillStyle"));
            if(style.getStyle("fillStyle")!="rgba(0,0,0,0)"){
                this.canvasse[ref]=new kPolygon(points,style);
            }else{
                this.canvasse[ref]=new kPolyline(points,style);
            }
            this.canvasse[ref].init(mapObj);
        }
        for(var ref in this.nodes){
            var point=this.nodes[ref]["point"];
            var color=this.nodes[ref]["color"];
            //console.log(color);
            if(color){
                ...
                style.addStyle("fillStyle",color);
                style.addStyle("strokeStyle","black");
                this.canvasse[ref]=new kCircle(point,"3px",style,this.nodes[ref]);
                this.canvasse[ref].init(mapObj);
            }   
        }
    }
    ...
    this.addOSM=function(dom){
        if(!dom)return;
                var cs=dom.documentElement.childNodes;
    ...
    this.loadOSM=function(url){
        var xmlhttp=new XMLHttpRequest();
        xmlhttp.open("GET","/iphonemapproxy/proxy.php?url="+url,false);
        var that=this;
        /*
        xmlhttp.onreadystatechange=function(){
            if(xmlhttp.readyState==4){
                that.addOSM(xmlhttp.responseXML);
            }
        }
        */
        xmlhttp.send(null);
        if(xmlhttp.readyState==4){
            this.addOSM(xmlhttp.responseXML);
        }
    }

    this.add=function(dom){
        if(!dom)return;
        if(dom.documentElement.tagName=="osm"){
            this.addOSM(dom);
        }
        if(dom.documentElement.tagName=="osmChange"){
            var cs=dom.documentElement.childNodes;
            var ids=new Array();
            for(var i=0; i < cs.length;i++){
                var c=cs.item(i);
                ...
                for(var j=0; j < els.length;j++){
                    ...
                    if(el.tagName=="way"){
                        ...
                        //this.loadOSM("/api/0.6/way/"+id+"/full"); 
                        var nodes=el.childNodes;
                        for(var h=0;h<nodes.length;h++){
                            var node=nodes.item(h);
                            if(el.nodeType!=1)continue;
                            if(node.tagName=="nd"){
                    ...
                    if(el.tagName=="node"){
                        if(el.getAttribute("version")==1){
                            this.addNode(el,"red");
                        }else{
                            this.addNode(el,"yellow");
                        }
                    }
                }
            }
            if(ids.length >0){
                var l=ids.length;
                var maxLength=100;  //url must be less than 2048 characters otherwise 414 error
                var m=Math.floor(l / maxLength);
                for(var i=0; i< Math.ceil(l / maxLength);i++){
                    var idsString="";
                    for(var j=i*maxLength; j < (i+1)*maxLength;j++){
                        if(!ids[j])break;
                        if(idsString==""){
                            idsString=ids[j];
                        }else{
                    ...
                    this.loadOSM("/api/0.6/nodes?nodes="+idsString);    
                }
            }
                        for(var i=0; i < cs.length;i++){
                                var c=cs.item(i);
...
function editor(map,osm){
    ...
    this.down=function(evt){
        var target=evt.target;
        if(target.getAttribute("osm")=="point"){
            this.moveElement=target;
            this.moveElement.setAttribute("fill","red");
            if(this.moveElement.param){
                this.showTags(this.moveElement.param["tags"]);
            }else{
                this.showTags(null);
            }
        }
        if(target.getAttribute("osm")=="way"){
            var points=target.thisobj.points;
            for(var i=0; i < this.circles.length;i++){
                this.map.removeOverlay(this.circles[i]);
                delete this.circles[i].kpoint;
            }
            for(var i=0; i < points.length;i++){
                ...
                var circle=new kCircle(p,"4px",style);
                circle.kpoint=p;
                this.map.addOverlay(circle);
                //circle.init(this.map);
                //circle.render();
    ...
    this.move=function(evt){
        if(this.moveElement){
            var x=this.map.pageX(evt);
            var y=this.map.pageY(evt);
            this.moveElement.setAttribute("cx",x);
            this.moveElement.setAttribute("cy",y);
            var p=this.moveElement.kpoint;
                        var movedpoint=this.map.XYTolatlng(x,y);
                        p.setLat(movedpoint.getLat());
                        p.setLng(movedpoint.getLng());

            var memberOf=this.moveElement.kpoint.getMemberOfArray();
    ...
    this.showTags=function(tags){
        ...
        if(!tags)return;
        for(var i=0;i<tags.length;i++){
            this.addLine(tags[i][0],tags[i][1]);
        }
    }
    this.addLine=function(k,v){
        var tr=document.createElement("tr");

        var td=document.createElement("td");
        var tn=document.createTextNode(k);
        td.appendChild(tn);
        tr.appendChild(td);
    ...
    var mapdiv=map.map.parentNode;
    map.overlayDiv.parentNode.removeChild(map.overlayDiv);
    Event.attach(mapdiv, "mousedown", this.down, this, false);
    Event.attach(mapdiv, "mousemove", this.move, this, false);
    Event.attach(mapdiv, "mouseup", this.up, this, false);
}
function debug(mapObj){
    this.mapObj=mapObj;
    ...
    this.init=function(div){
        ...
        div.appendChild(this.debugLine);
...
//      alert(this.mapObj.map.parentNode.tagName);
        for(var i=1; i <= maxZoom;i++){
            //if(Math.floor(this.mapObj.getZoom())==i){
            if(this.mapObj.intZoom==i){
                this.trArray[i].style.color="green";
            }else{
                this.trArray[i].style.color="red";
            }
            if(this.mapObj.visibleZoom==i){
                this.trArray[i].style.backgroundColor="lightgreen";
            }else{
                this.trArray[i].style.backgroundColor="white";
            }
            this.clear(i);
            try{
                var lc=this.mapObj.layers[i]["loadComplete"];
                this.addValue(i,i);
                this.addValue(i,lc);
            }catch(e){};
            var nl=this.countImages(i);
            this.addValue(i,nl);
            try{
            var vis=this.mapObj.layers[i]["layerDiv"].style.visibility;
            this.addValue(i,vis);
            }catch(e){}
        }
        this.debugLine.textContent="ld: "+this.mapObj.loadingZoomLevel+" zoom:"+this.mapObj.getZoom()+" visi:"+this.mapObj.visibleZoom+" f/s:"+this.mapObj.framesCounter;
    }
    this.countImages=function(layer){
        var total=0;
        var notLoaded=0;

        if(!this.mapObj.layers[layer]) return "";

        for (var vimg in this.mapObj.layers[layer]["images"]) {
            total++;
            try{
            var img=this.mapObj.layers[layer]["images"][vimg]["img"];
            if(img.getAttribute("loaded")!="yes"){
                notLoaded++;
        ...
        if(notLoaded==0){
            if(layer==this.mapObj.intZoom){
                if(!this.mapObj.layers[layer]["loadComplete"]){
//                  alert(layer);
                this.mapObj.layers[layer]["layerDiv"].style.visibility = "";
                }
            }
        }
        return total+" : "+notLoaded;
    }
...
//                      console.log("----name---");
    ...
    this.addStyle = function (name, value) {
        ...
        for(var i=0; i < this.arrayStyle.length;i++){
                        //console.log(name,this.arrayStyle[i][0]);
                if(this.arrayStyle[i][0]==name){
                        //console.log("uuuu");
                        //console.log(name,this.arrayStyle[i][0]);
                        this.arrayStyle.splice(i, 1);
                        //delete this.arrayStyle[i];
                }
        }
    ...
    this.getStyle=function(name){
        for(var i=0; i < this.arrayStyle.length;i++){
                ...
                //console.log(this.arrayStyle[i][0],name);
                if(this.arrayStyle[i][0]==name){
            return this.arrayStyle[i][1];
        }
    }
    }
...
kCircle.prototype = new kVector();
kPolyline.prototype = new kVector();

function kPolyline(points,style,close){
    this.close=false;
    if(close){
        this.close=close;
    }
    ...
    this.setPoints=function(points){
        ...
        this.render=function(){
                switch(this.backend){
                        case "svg": this.renderSVG(); break;
                        case "canvas": this.renderCanvas();break;
                        case "vml": this.renderVML();break;
                }
        }
        this.clear=function(){
                if(this.path){
    ...
    this.cutOut=function(points){
        this.holes.push(points);
    }
        this.renderSVG=function(){
        if(!this.path){
            this.path=document.createElementNS(svgns,"path");
            this.path.setAttribute("osm","way");
            this.path.thisobj=this;
        }
                this.setStyle(this.path,this.style);
                var d=this.dSVG(this.points);
        if(this.close){
            d=d+" z ";
            for(var i=0; i < this.holes.length;i++){
                this.path.setAttribute("fill-rule","evenodd");
                d=d+this.dSVG(this.holes[i]);
                d=d+" z ";
            }
        }else{
            this.path.setAttribute("fill","none");
            ...
            if(!this.path.parentNode){
                this.vectorEl.appendChild(this.path);
            }
                }
        }
    this.dSVG=function(points){
        var delta=1;
        var lastX=0;
        var lastY=0;
        for(var i=0; i < points.length;i++){
                        var point=points[i];
                        var xy=this.mapObj.latlngToXY(point);
                        if(i==0||(Math.abs(xy.x - lastX) > delta) ||(Math.abs(xy.y - lastY) > delta)||this.points.length==i+1){
                if(i==0){
                    var d="M"+xy.x+","+xy.y;
                }else{
                    d=d+" L"+xy.x+","+xy.y;
        ...
        this.renderCanvas=function(){
                this.setStyle(this.vectorEl,this.style);
        this.ctx.globalCompositeOperation = 'destination-over';
        this.dCanvas(this.points);
        if(this.close){
            this.ctx.closePath();
            this.ctx.fill();
        }
    ...
    this.renderVML=function(){
        this.clear();
                this.path=document.createElement("v:polyline");
                //var path=document.createElement("v:shape");
        this.vectorEl.appendChild(this.path);
                this.setStyle(this.path,this.style);
                var d=this.dVML(this.points);
        this.path.setAttribute("filled",this.close);
        this.path.setAttribute("clip-rule","nonzero");

                if(d!=""){
                        //path.style.position="absolute";
                        //path.setAttribute("path",d+" e");
            this.path.points.value=d;
                        //path.setAttribute("fillcolor","green");
                        //path.setAttribute("stroked","black");
                        this.path.setAttribute("filled",this.close);
...
function kCircle(point,radius,style,param){
    this.point=point;
        this.radius=radius;
        this.style=style;
    this.circle=null;   
    ...
    this.moveTo=function(point){
        ...
        this.circle.setAttribute("osm","point");
                this.circle.setAttribute("fill","none");
                this.setStyle(this.circle,this.style);
                //this.circle.setAttribute("r",r);
        this.makeRadius(this.radius);
                var xy=this.mapObj.latlngToXY(this.point);

                this.circle.setAttribute("cx",xy.x);
                this.circle.setAttribute("cy",xy.y);
        this.circle.kpoint=this.point;
        this.circle.param=param;
        this.circle.obj=this;
        if(newCircle){
            this.vectorEl.appendChild(this.circle);
        }
        }
    this.renderCanvas=function(){
                this.setStyle(this.vectorEl,this.style);
            ...
            var r=this.makeRadius(this.radius);
                        //var r=parseFloat(this.radius);
                }else{
                        try{
                        var r=this.radius;
                        var helperpoint=new kPoint(this.point.getLat(),this.point.getLng() +this.radius);
                        var xy2=this.mapObj.latlngToXY(helperpoint);
                        var r=xy2.x - xy.x;
                        }catch(e){
                        //alert(xy.x+" : "+xy2.x+" : "+r);
                        }
                ...
                this.ctx.arc(xy.x,xy.y,r,0,Math.PI*2,true);
                this.ctx.fill();
                this.ctx.stroke();
    ...
    this.renderVML=function(){
        ...
        this.circle=document.createElement("v:oval");
                var xy=this.mapObj.latlngToXY(this.point);
        //var s="left:"+(xy.x-this.radius)+";width:"+(this.radius*2)+";top:"+(xy.y-this.radius)+";height:"+(this.radius*2);
        //alert(s);
                //circle.setAttribute("style",s);
        var r=this.makeRadius(this.radius);
        this.circle.style.left=xy.x-r;
        this.circle.style.top=xy.y-r;
        this.circle.style.width=r*2;
        this.circle.style.height=r*2;
        this.circle.style.position="absolute";
                this.circle.setAttribute("fillcolor","yellow");
                this.circle.setAttribute("stroked","black");
                this.vectorEl.appendChild(this.circle);
        //alert(this.vectorEl.innerHTML);
    }
    this.makeRadius=function(radius){
        this.radius=radius;     
                if(isNaN(radius)){
                        ...
                        try{
                        var r=radius;
                        var helperpoint=new kPoint(this.point.getLat(),this.point.getLng() +radius);
            var xy=this.mapObj.latlngToXY(this.point);
                        var xy2=this.mapObj.latlngToXY(helperpoint);
                        var r=xy2.x - xy.x;
                        }catch(e){
